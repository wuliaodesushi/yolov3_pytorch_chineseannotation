...
class ListDataset(Dataset):
    def __init__(self, list_path, img_size=416, augment=False, multiscale=True, normalized_labels=True):
        ...
        '''
        self.img_size 图片的resize
        self.max_objects 每张图片最大预测图片数
        self.augment 是否图片增强
        self.multiscale 是否多尺度训练
        self.normalized_labels 是否是归一化的标签
        self.min_size 多尺度训练的最小尺度
        self.max_size 多尺度训练的最大尺度
        self.batch_count 批量计数
        '''
        self.img_size = img_size
    ...
    def collate_fn(self, batch):
        '''自定义dataloader中，批量读取数据的功能，实现多尺度训练'''
        ...
        ''' torch.stack沿着一个新维度对输入张量序列进行连接。 序列中所有的张量都应该为相同形状。'''
